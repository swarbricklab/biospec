...
def select_with_arrows(options: dict, prompt_text: str = "Select an option", default_key: str = None) -> str:
    ...
    def create_selection_panel():
        ...
        table.add_row("", "")
        table.add_row("", "[dim]Use Up/Down to navigate, Enter to select, Esc to cancel[/dim]")
...
def callback(ctx: typer.Context):
    ...
    if ctx.invoked_subcommand is None and "--help" not in sys.argv and "-h" not in sys.argv:
        show_banner()
        console.print(Align.center("[dim]Run 'biospec --help' for usage information[/dim]"))
        console.print()
...
def download_template_from_github(download_dir: Path, *, script_type: str = "sh", verbose: bool = True, show_progress: bool = True, client: httpx.Client = None, debug: bool = False, github_token: str = None) -> Tuple[Path, dict]:
    """Download the latest BioSpec template from GitHub releases."""
    repo_owner = "swarbricklab"
    repo_name = "BioSpec"
    if client is None:
        client = httpx.Client(verify=ssl_context)
    ...
    pattern = f"biospec-template-{script_type}"
    matching_assets = [
    ...
    # Use API URL for private repo support (browser_download_url doesn't work with tokens)
    ...
    if debug:
        console.print(f"[dim]API URL: {download_url}[/dim]")
        console.print(f"[dim]Browser URL: {asset.get('browser_download_url', 'N/A')}[/dim]")

    zip_path = download_dir / filename
    if verbose:
        console.print(f"[cyan]Downloading template...[/cyan]")

    # For asset downloads, need Accept header to get binary content
    download_headers = _github_auth_headers(github_token)
...
def download_and_extract_template(project_path: Path, script_type: str, is_current_dir: bool = False, *, verbose: bool = True, tracker: StepTracker | None = None, client: httpx.Client = None, debug: bool = False, github_token: str = None) -> Path:
...
def ensure_executable_scripts(project_path: Path, tracker: StepTracker | None = None) -> None:
    """Ensure POSIX .sh scripts under .biospec-scripts (recursively) have execute bits (no-op on Windows)."""
    if os.name == "nt":
        return  # Windows: skip silently
    scripts_root = project_path / ".biospec-scripts"
...
def init(
    ...
    """
    Initialize a new BioSpec project from the latest template.

    This command will:
    1. Check that required tools are installed (git is optional)
    2. Download the BioSpec template from GitHub
    3. Extract the template to a new project directory or current directory
    4. Initialize a fresh git repository (if not --no-git and no existing repo)
    5. Set up GitHub Copilot prompts and agent files
    ...
    tracker = StepTracker("Initialize BioSpec Project")

    sys._biospec_tracker_active = True
    ...
    with Live(tracker.render(), console=console, refresh_per_second=8, transient=True) as live:
        ...
        try:
            ...
            download_and_extract_template(project_path, selected_script, here, verbose=False, tracker=tracker, client=local_client, debug=debug, github_token=github_token)
    ...
    steps_lines.append(f"{step_num}. Open the project in VS Code with GitHub Copilot enabled")
    step_num += 1
    ...
    steps_lines.append("")
    steps_lines.append("   [bold cyan]Track[/bold cyan]")
